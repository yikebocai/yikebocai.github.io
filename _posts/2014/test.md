...
这是一份 Cmd Markdown 的欢迎稿兼使用说明，请保留，如需撰写 **新稿件**，点击顶部工具栏右侧的 `新文稿` 或者使用快捷键 `Ctrl+Alt+N`。
...
* `发布文稿`：将当前的文稿生成固定链接，在网络上发布，分享
* `新建文稿`：开始撰写一篇新的文稿
* `删除文稿`：删除当前的文稿
* `导出文稿`：将当前的文稿转化为 Markdown 文本或者 Html 格式，并导出到本地
...
在 **阅读工具栏** 点击 `阅读模式` 图标或者按下 `Ctrl+Alt+M` 随即进入独立的阅读模式界面，我们在版面渲染上的每一个细节：字体，字号，行间距，前背景色都倾注了大量的时间，努力提升阅读的体验和品质。

### 9. 标签、分类和搜索

在编辑区任意行的列首位置输入以下格式的文字可以标签当前文档：
...
在您使用 Cmd Markdown 记录，创作，整理，阅读文稿的同时，我们不仅希望它是一个有力的工具，更希望您的思想和知识通过这个平台，连同优质的阅读体验，将他们分享给有相同志趣的人，进而鼓励更多的人来到这里记录分享他们的思想和知识，尝试点击 `发布(Ctrl+Alt+P)` 图标发布这份文档给好友吧！
